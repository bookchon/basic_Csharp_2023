...
namespace wf09_dbhandling
{
    public partial class FrmMain : Form
    {
        ...
        private void FrmMain_Load(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(connectionString); // 연결 문자열 없이 객체 생성 안 됨
            // 3. 객체를 통해서 DB 연결
            conn.Open();
            ...
            string selQuery = @"SELECT CustomerID
                                      ...
                                      ,Fax
                                  FROM Customers";
            // SqlCommand celCmd = new SqlCommand(selQuery, conn);
            // celCmd.Parameters.Clear();
            SqlDataAdapter adapter = new SqlDataAdapter(selQuery, conn);

            // - 리더객체 생성, 값을 넘겨줌
            // SqlDataReader reader = celCmd.ExecuteReader();

            // - 데이터리더에 있는 데이터를 데이터셋으로 보내기
            ...
            // 7. 데이터 그리드 뷰에 바인딩 하기 위한 Binding Source 생성
            BindingSource source = new BindingSource();

            // 8. 데이터 그리드 뷰의 DataSource에 데이터 셋을 할당
